inherited FCadEmprestimo: TFCadEmprestimo
  ...
  inherited qConsulta: TIBQuery
    SQL.Strings = (
        ...
        'select EMPR_ID,EMPR_CREDOR_ID,EMPR_DESCRICAO,EMPR_DATA,EMPR_VALO' +
        'R,EMPR_DH_CA,'
      '       CREDOR,VALOR_PAGO,EMPR_VALOR - VALOR_PAGO SALDO,PAGO'
      
        '  from (select EMPR_ID,EMPR_CREDOR_ID,EMPR_DESCRICAO,EMPR_DATA,E' +
        'MPR_VALOR,EMPR_DH_CA,'
      
        '               CREDOR,VALOR_PAGO,case when(EMPR_VALOR<=VALOR_PAG' +
        'O) then 1 else 0 end pago'
      
        '          from (select EMPRESTIMO.EMPR_ID,EMPRESTIMO.EMPR_CREDOR' +
        '_ID,'
      
        '                       EMPRESTIMO.EMPR_DESCRICAO,EMPRESTIMO.EMPR' +
        '_DATA,'
      
        '                       EMPRESTIMO.EMPR_VALOR,EMPRESTIMO.EMPR_DH_' +
        'CA,'
      '                       PESSOA.pes_nome_razao credor,'
      
        '                       (SELECT COALESCE(SUM(parcelas_detalhe.det' +
        '_valor),0)'
      '                         FROM parcelas_detalhe'
      
        '                        WHERE parcelas_detalhe.det_emprestimo_id' +
        '=EMPRESTIMO.empr_id'
      
        '                          AND ((parcelas_detalhe.det_datapgto is' +
        ' not null)'
      '                           OR ((select parcelas.par_datapgto'
      '                                 from parcelas'
      
        '                                where parcelas.par_id=parcelas_d' +
        'etalhe.det_par_id'
      
        '                                  and parcelas.par_pago=1) is no' +
        't null))) valor_pago'
      '                  FROM EMPRESTIMO'
      
        '                 JOIN PESSOA ON (PESSOA.pes_id=EMPRESTIMO.empr_c' +
        'redor_id)'
      
        '                 where ((COALESCE(emprestimo.empr_inativo,'#39'N'#39') =' +
        ' :INATIVO1) or (:INATIVO2 = '#39' '#39'))'
      ...
      '                                 from usuario_visao'
        ...
        'HO=EMPRESTIMO.empr_usu_id )))'
      
        ' WHERE ((PAGO = :SITUACAO1) OR (:SITUACAO2 = 3)) --0=ABERTO 1=FE' +
        'CHADO 3=TODOS'
      ' ORDER BY EMPR_DATA')
    ParamData = <
      item
